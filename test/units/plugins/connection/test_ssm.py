...
class TestConnectionBaseClass(unittest.TestCase):
    ...
    def test_plugins_connection_ssm__build_command(self):
        pc = PlayContext()
        new_stdin = StringIO()
        conn = connection_loader.get('ssm', pc, new_stdin)
        conn._build_command('ssm')

    def test_plugins_connection_ssm_exec_command(self):
        pc = PlayContext()
        new_stdin = StringIO()
        conn = connection_loader.get('ssm', pc, new_stdin)

        conn._build_command = MagicMock()
        conn._build_command.return_value = 'ssm something something'
        conn._run = MagicMock()
        conn._run.return_value = (0, 'stdout', 'stderr')
        conn.get_option = MagicMock()
        conn.get_option.return_value = True

        res, stdout, stderr = conn.exec_command('ssm')
        res, stdout, stderr = conn.exec_command('ssm', 'this is some data')

    def test_plugins_connection_ssm__examine_output(self):
        pc = PlayContext()
        new_stdin = StringIO()
    ...
    def test_plugins_connection_ssm_put_file(self, mock_ospe, mock_sleep):
        pc = PlayContext()
        new_stdin = StringIO()
        conn = connection_loader.get('ssm', pc, new_stdin)
        conn._build_command = MagicMock()
        ...
        C.ANSIBLE_SSM_RETRIES = 9

        # Test with C.DEFAULT_SCP_IF_SSM set to smart
        # Test when SFTP works
        C.DEFAULT_SCP_IF_SSM = 'smart'
        ...
        # test with C.DEFAULT_SCP_IF_SSM enabled
        C.DEFAULT_SCP_IF_SSM = True
        conn.put_file('/path/to/in/file', '/path/to/dest/file')
        conn._bare_run.assert_called_with('some command to run', None, checkrc=False)
        ...
        # test with C.DEFAULT_SCP_IF_SSM disabled
        C.DEFAULT_SCP_IF_SSM = False
        expected_in_data = b' '.join((b'put', to_bytes(shlex_quote('/path/to/in/file')), to_bytes(shlex_quote('/path/to/dest/file')))) + b'\n'
        conn.put_file('/path/to/in/file', '/path/to/dest/file')
        conn._bare_run.assert_called_with('some command to run', expected_in_data, checkrc=False)
    ...
    @patch('time.sleep')
    def test_plugins_connection_ssm_fetch_file(self, mock_sleep):
        pc = PlayContext()
        new_stdin = StringIO()
        conn = connection_loader.get('ssm', pc, new_stdin)
        conn._build_command = MagicMock()
...
def mock_run_env(request, mocker):
    ...
    conn._load_name = 'ssm'
    conn.ssmpass_pipe = [MagicMock(), MagicMock()]
...
class TestSSMConnectionRun(object):
    # FIXME:
    ...
    def test_no_escalation(self):
        ...
        return_code, b_stdout, b_stderr = self.conn._run("ssm", "this is input data")
    ...
    def test_with_password(self):
        # test with a password set to trigger the ssmpass write
        ...
        return_code, b_stdout, b_stderr = self.conn._run(["ssm", "is", "a", "cmd"], "this is more data")
        assert return_code == 0
    ...
    def test_pasword_without_data(self):
        ...
        return_code, b_stdout, b_stderr = self.conn._run("ssm", "")
        assert return_code == 0
        assert b_stdout == b'some data'
        assert b_stderr == b''
        assert self.mock_selector.register.called is True
...
class TestSSMConnectionRetries(object):
    def test_incorrect_password(self, monkeypatch):
        ...
        self.conn._build_command.return_value = [b'ssmpass', b'-d41', b'ssm', b'-C']
        self.conn.get_option = MagicMock()
        self.conn.get_option.return_value = True

        exception_info = pytest.raises(AnsibleAuthenticationFailure, self.conn.exec_command, 'ssmpass', 'some data')
    ...
    def test_retry_then_success(self, monkeypatch):
        monkeypatch.setattr(C, 'HOST_KEY_CHECKING', False)
        monkeypatch.setattr(C, 'ANSIBLE_SSM_RETRIES', 3)
        ...
        self.conn._build_command = MagicMock()
        self.conn._build_command.return_value = 'ssm'
        self.conn.get_option = MagicMock()
        self.conn.get_option.return_value = True

        return_code, b_stdout, b_stderr = self.conn.exec_command('ssm', 'some data')
        assert return_code == 0
        assert b_stdout == b'my_stdout\nsecond_line'
        assert b_stderr == b'my_stderr'
    ...
    def test_multiple_failures(self, monkeypatch):
        monkeypatch.setattr(C, 'HOST_KEY_CHECKING', False)
        monkeypatch.setattr(C, 'ANSIBLE_SSM_RETRIES', 9)
        ...
        pytest.raises(AnsibleConnectionFailure, self.conn.exec_command, 'ssm', 'some data')
    ...
    def test_abitrary_exceptions(self, monkeypatch):
        ...
        pytest.raises(Exception, self.conn.exec_command, 'ssm', 'some data')
        assert self.mock_popen.call_count == 10
